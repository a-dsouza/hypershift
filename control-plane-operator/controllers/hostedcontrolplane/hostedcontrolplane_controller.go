...
func (r *HostedControlPlaneReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if r.OperateOnReleaseImage != "" && r.OperateOnReleaseImage != util.HostedControlPlaneReleaseImage(hostedControlPlane) {
		r.Log.Info("releaseImage is %s, but this operator is configured for %s, skipping reconciliation", util.HostedControlPlaneReleaseImage(hostedControlPlane), r.OperateOnReleaseImage)
		return ctrl.Result{}, nil
	}
...
func (r *HostedControlPlaneReconciler) LookupReleaseImage(ctx context.Context, hcp *hyperv1.HostedControlPlane) (*releaseinfo.ReleaseImage, error) {
	...
	defer lookupCancel()
	return r.ReleaseProvider.Lookup(lookupCtx, util.HostedControlPlaneReleaseImage(hcp), pullSecret.Data[corev1.DockerConfigJsonKey])
