...
func GetControlPlaneOperatorImage(ctx context.Context, hc *hyperv1.HostedCluster, releaseProvider releaseinfo.Provider, hypershiftOperatorImage string, pullSecret []byte) (string, error) {
	...
	releaseInfo, err := releaseProvider.Lookup(ctx, hostedClusterReleaseImage(hc), pullSecret)
	if err != nil {
		return "", err
	}
				...
				Containers: []corev1.Container{
					{
						...
						Env: []corev1.EnvVar{
							...
							},
							{
								...
								Value: hostedClusterReleaseImage(hc),
							},
							{
...
func computeClusterVersionStatus(clock clock.WithTickerAndDelayedExecution, hcluster *hyperv1.HostedCluster, hcp *hyperv1.HostedControlPlane) *hyperv1.ClusterVersionStatus {
	...
	releaseImage := hostedClusterReleaseImage(hcluster)
	...
	hcpRolloutComplete := (hyperutil.HostedControlPlaneReleaseImage(hcp) == hcp.Status.ReleaseImage) && (version.Desired.Image == hcp.Status.ReleaseImage)
...
func (r *HostedClusterReconciler) validateReleaseImage(ctx context.Context, hc *hyperv1.HostedCluster) error {
	...
	if hc.Status.Version != nil && hc.Status.Version.Desired.Image != hostedClusterReleaseImage(hc) {
...
func isProgressing(hc *hyperv1.HostedCluster, releaseImage *releaseinfo.ReleaseImage) (bool, error) {
	...
	if hc.Status.Version == nil || hostedClusterReleaseImage(hc) != hc.Status.Version.Desired.Image {
...
func (r *HostedClusterReconciler) lookupReleaseImage(ctx context.Context, hcluster *hyperv1.HostedCluster) (*releaseinfo.ReleaseImage, error) {
	...
	return r.ReleaseProvider.Lookup(ctx, hostedClusterReleaseImage(hcluster), pullSecretBytes)
}
...
func isUpgradeable(hcluster *hyperv1.HostedCluster, releaseImage *releaseinfo.ReleaseImage) (bool, string, error) {
	if hcluster.Status.Version == nil || hcluster.Status.Version.Desired.Image == hostedClusterReleaseImage(hcluster) {
	...
	if exists {
		if upgradeImage != hostedClusterReleaseImage(hcluster) {
...
func hostedClusterReleaseImage(hcluster *hyperv1.HostedCluster) string {
	if hcluster.Spec.ControlPlaneRelease != nil {
